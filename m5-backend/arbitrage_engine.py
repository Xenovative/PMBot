...
class ArbitrageEngine:
    ...
    def _try_buy_one_side(self, clob_client, token_id: str, amount_usd: float,
        ...
        debug_ctx = {"maker": "", "taker": "", "maker_amt": "", "taker_amt": ""}
        try:
            ...
            try:
                maker = getattr(signed, "order", {}).get("maker") if isinstance(getattr(signed, "order", None), dict) else getattr(getattr(signed, "order", None), "maker", "")
                taker = getattr(signed, "order", {}).get("taker") if isinstance(getattr(signed, "order", None), dict) else getattr(getattr(signed, "order", None), "taker", "")
                maker_amt = getattr(signed, "order", {}).get("makerAmount") if isinstance(getattr(signed, "order", None), dict) else getattr(getattr(signed, "order", None), "makerAmount", "")
                taker_amt = getattr(signed, "order", {}).get("takerAmount") if isinstance(getattr(signed, "order", None), dict) else getattr(getattr(signed, "order", None), "takerAmount", "")
                debug_ctx.update({"maker": maker, "taker": taker, "maker_amt": maker_amt, "taker_amt": taker_amt})
                # Minimal payload log
                self.status.add_log(
                    f"  🧾 {side_label} order payload | maker={maker} taker={taker} makerAmt={maker_amt} takerAmt={taker_amt}"
                )
            except Exception:
                pass
            resp = clob_client.post_order(signed, OrderType.FOK)
            self.status.add_log(
                f"  📋 {side_label} post_order 回應: {str(resp)[:200]} | amount_usd={amount_usd:.2f} token={token_id}"
            )
        except Exception as e:
            # Capture richer error info from PolyApiException if available
            err_detail = getattr(e, "error_msg", None) or getattr(e, "error_msg", None)
            if hasattr(e, "error_msg") and not err_detail:
                try:
                    err_detail = e.error_msg
                except Exception:
                    err_detail = None
            last_error = err_detail or str(e)
            self.status.add_log(
                f"  ⚠️ {side_label} FOK 失敗: {last_error[:200]} | amount_usd={amount_usd:.2f} token={token_id}"
            )
            return {"success": False, "error": last_error[:200], "shares": 0, "price": price}
