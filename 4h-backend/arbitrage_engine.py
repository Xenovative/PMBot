...
class ArbitrageEngine:
    ...
    def _load_pending_unwinds(self) -> List[Dict[str, Any]]:
        raw_pending = trade_db.kv_get("4h_backend_pending_unwinds", "[]")
        try:
    ...
    def _save_pending_unwinds(self, pending_unwinds: List[Dict[str, Any]]):
        try:
            trade_db.kv_set("4h_backend_pending_unwinds", json.dumps(pending_unwinds, ensure_ascii=False))
        except Exception as e:
            ...
            if not self.config.dry_run:
                try:
                    ...
                    if clob_client:
                        ...
                        await self._try_unwind_position(clob_client, h.token_id, shares, sell_price, f"到期平倉-{h.side}", holding_bid_levels)
                except Exception as e:
    ...
    async def check_bargain_opportunities(self, markets: List[MarketInfo]) -> List[Dict[str, Any]]:
        ...
        for market in markets:
            ...
            if unpaired:
                ...
                window_limit = getattr(self.config, "bargain_open_time_window_seconds", 10800)
