"""
Polymarket 套利機器人 - 4 小時 Up or Down 市場版本 - FastAPI 後端
"""
import asyncio
import json
import os
...
app = FastAPI(title="Polymarket 4 小時套利機器人")
...
async def bot_loop():
    ...
    engine.status.add_log(f"🚀 4 小時套利機器人啟動 | 模式: {engine.status.mode}")
    ...
    try:
        while engine.status.running:
            ...
            if not all_markets:
                engine.status.add_log("⏳ 未找到活躍 4 小時市場，5 秒後重試...")
            ...
            if not valid_markets:
                engine.status.add_log("⏳ 無符合條件的 4 小時市場，5 秒後重試...")
                engine.status.active_markets = []
                await broadcast({"type": "status", "data": engine.status.to_dict()})
                for _ in range(5):
                    if not engine.status.running:
...
async def get_current_config(_user=Depends(auth.require_auth)):
    return {
        ...
        "bargain_open_time_window_seconds": getattr(config, "bargain_open_time_window_seconds", 10800),
...
async def get_price(crypto: str, _user=Depends(auth.require_auth)):
    ...
    if not market:
        return {"error": f"未找到 {crypto.upper()} 的活躍 4 小時市場"}
