...
class ArbitrageEngine:
    ...
    def _load_pending_unwinds(self) -> List[Dict[str, Any]]:
        raw_pending = trade_db.kv_get("daily_backend_pending_unwinds", "[]")
        try:
    ...
    def _save_pending_unwinds(self, pending_unwinds: List[Dict[str, Any]]):
        try:
            trade_db.kv_set("daily_backend_pending_unwinds", json.dumps(pending_unwinds, ensure_ascii=False))
        except Exception as e:
    ...
    async def test_connection(self, markets: List[MarketInfo]):
        ...
        # Floor to whole shares — CLOB balance check uses on-chain integer amounts
        shares = math.floor(shares)
        if shares < 1:
            self.status.add_log("⚠️ 測試買入股數不足 1，無法平倉")
            return
        self.status.add_log(f"✅ 測試買入成功 {shares} 股 @ {buy_price:.4f}，等待結算後平倉...")
        # Wait for on-chain settlement before selling
