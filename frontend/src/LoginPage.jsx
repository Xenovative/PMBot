...
import { Zap, Lock, Shield, Eye, EyeOff, KeyRound, Smartphone } from 'lucide-react'
        ...
        <div className="cyber-panel p-8">
          ...
          {mode === 'login' && (
            <form onSubmit={handleLogin} className="space-y-6">
              <div className="text-center mb-2">
                ...
                <p className="text-xs text-gray-500 mt-1">輸入管理員密碼{authStatus?.totp_enabled ? ' + 驗證碼' : ''}</p>
              </div>

              <div>
                <label className="text-xs text-neon-cyan/50 block mb-1.5 uppercase tracking-wider">密碼</label>
                <div className="relative">
                  <input
                    type={showPassword ? 'text' : 'password'}
                    value={password}
              ...
              {authStatus?.totp_enabled && (
                <div>
                  <label className="text-xs text-neon-cyan/50 block mb-1.5 uppercase tracking-wider">驗證碼</label>
                  <div className="flex items-center space-x-2">
                    <input
                      ...
                      placeholder="6 位數"
                      className="w-full bg-black/40 border border-neon-cyan/15 rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-neon-cyan/50 focus:shadow-neon-cyan transition-all text-gray-200"
                    />
          ...
          {mode === 'totp' && (
            <form onSubmit={handleTotpSubmit} className="space-y-6">
              <div className="text-center mb-2">
                <Smartphone className="w-6 h-6 text-neon-cyan mx-auto mb-2" />
                <h2 className="text-lg font-cyber neon-text-cyan">輸入驗證碼</h2>
                <p className="text-xs text-gray-500 mt-1">請輸入 6 位數 TOTP 驗證碼</p>
              </div>
