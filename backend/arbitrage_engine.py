...
class ArbitrageEngine:
    ...
    def check_arbitrage(self, market: MarketInfo, price_info: PriceInfo) -> ArbitrageOpportunity:
        ...
        # 計算潛在利潤
        investment = total_cost * order_size
    ...
    def _calculate_safe_order_size(self, price_info: PriceInfo, desired_size: float) -> float:
        """
        根據訂單簿深度計算安全的下單數量
        如果任一側 USD 金額 < $1，跳過交易（Polymarket 最低 $1）
        """
        MIN_ORDER_USD = 1.0
    ...
    def _try_buy_one_side(self, clob_client, token_id: str, amount_usd: float,
        ...
        """
        嘗試用 FOK 買入一側代幣
        FOK = 全部成交或全部取消，適合高波動市場
        返回 {success, response, shares_bought}
        """
        from py_clob_client.clob_types import MarketOrderArgs, OrderType
        from py_clob_client.order_builder.constants import BUY

        # 估算買入股數: BUY amount = USD, 股數 ≈ USD / price
        ...
        try:
            ...
            error_msg = str(e)
            self.status.add_log(
                f"  ⚠️ {side_label} FOK 失敗: {error_msg[:150]}"
            )
            return {"success": False, "error": error_msg, "shares": 0, "price": price}
        ...
        if self.config.dry_run:
            ...
            try:
                ...
                if not first_result["success"]:
                    ...
                    half_size = max(round(order_size * 0.5, 2), 1.0)
                    if half_size < order_size:
                        self.status.add_log(f"  🔄 重試較小數量: {half_size}")
                        first_result = self._try_buy_one_side(
                            clob_client, first_token,
                            half_size * first_price,
